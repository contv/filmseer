...
const nFormatter = (num: number, digits: number) => {
  ...
  return (num / suffix[i].value).toFixed(digits).replace(rx, "$1") + suffix[i].symbol;
}

const MovieItem = (props: MovieItemProps & { className?: string }) => {
  const avgRating: number = parseFloat((props.cumulativeRating / props.numRatings).toFixed(1))
  ...
  return (
    <Card>
      ...
      />
      <CardContent>
        <Stars movieId={props.movieId} rating={avgRating} size="small" votable={false}/>
        <Typography variant="subtitle1" display="inline" >{avgRating}({formattedNumRatings})</Typography>
      </CardContent>
    </Card>
  );
}
